...
LawGPT AI is an AI-native Legal Operating System, not a chatbot. Built for the Sarvam Epoch Buildathon. Frontend-only scope: this team owns UI/UX; backend team owns agents, orchestration, APIs, auth, search, RAG, document processing, compliance engine, voice, storage, DB. Every component is built API-ready against mock JSON, never backend logic in the frontend.
