    ...
    try:
        ...
        if not firebase_admin._apps:
            if settings.GOOGLE_APPLICATION_CREDENTIALS:
                logger.info(f"Initializing Firebase using service account file: {settings.GOOGLE_APPLICATION_CREDENTIALS}")
        ...
        if settings.ENVIRONMENT in ["production", "staging"]:
            ...
            raise RuntimeError(f"Database initialization failed in {settings.ENVIRONMENT} environment: {e}")
        else:
            logger.warning("Continuing execution in development/testing mode without Firebase connection.")
            return None
...
def get_firestore_client() -> Client:
    ...
    if _db is None:
        ...
        if client is None:
            raise RuntimeError("Firestore is not initialized. Please verify credentials.")
        return client
    return _db
