...
def initialize_storage() -> storage.Client | None:
    ...
    try:
        if settings.GOOGLE_APPLICATION_CREDENTIALS:
            logger.info(f"Initializing Google Cloud Storage using service account: {settings.GOOGLE_APPLICATION_CREDENTIALS}")
            _storage_client = storage.Client.from_service_account_json(settings.GOOGLE_APPLICATION_CREDENTIALS)
        else:
            if settings.ENVIRONMENT in ["production", "staging"]:
                logger.info("Initializing Google Cloud Storage using Application Default Credentials (ADC)")
        ...
        if settings.ENVIRONMENT in ["production", "staging"]:
            raise RuntimeError(f"Storage initialization failed in {settings.ENVIRONMENT} environment: {e}")
        else:
            logger.warning("Continuing execution in development/testing mode without Google Cloud Storage.")
            return None
...
def get_storage_client() -> storage.Client:
    ...
    if _storage_client is None:
        ...
        if client is None:
            raise RuntimeError("GCS Client is not initialized. Please verify credentials.")
        return client
    return _storage_client
