...
def setup_teardown_history():
    ...
    test_history = Path("/Users/niteshsahu/Desktop/lawgpt-ai-os/backend/data/test_research_history.json")
...
def test_result_formatter():
    ...
    assert formatted["Detailed Explanation"] == "Explanation text"
    assert formatted["Applicable Law"] == "IPC"
    assert formatted["Relevant Sections"] == ["Sec 302"]
    assert formatted["Related Acts"] == ["Indian Penal Code"]
    assert formatted["References"] == ["Ref 1"]
    assert formatted["Confidence Score"] == 0.9
